...
function Pokeitem(itemsProps: itemsProps) {
  ...
  return (
    <li style={{ display: 'flex', justifyContent: 'left' }} data-testid='poke-item'>
      <input type="checkbox" onChange={handleSelection} checked={router.query.checked ? JSON.parse(router.query.checked as string).includes(itemsProps.item.name) : false} data-testid='checkbox'/>
      <button
